...
-(instancetype)initWithFrame:(CGRect)frame
{
    if(self = [super initWithFrame:frame])
    {
	   ...
	   if (self.delegate) {
		   if ([self.delegate respondsToSelector:@selector(cycleScrollViewItemCustomClass)]) {
			   Class aClass = [self.delegate cycleScrollViewItemCustomClass];
			   [self.collectionView registerClass:aClass forCellWithReuseIdentifier:NSStringFromClass(aClass)];
		   }
	   }
    }
    return self;
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (self.delegate) {
		if ([self.delegate respondsToSelector:@selector(cycleScrollViewItemCustomClass)]) {
			//实现这个方法进行注册cell
			Class aClass = [self.delegate cycleScrollViewItemCustomClass];
		     NSString *cellIdentifier = 	 NSStringFromClass(aClass);
			UICollectionViewCell *aCell = [collectionView dequeueReusableCellWithReuseIdentifier:cellIdentifier forIndexPath:indexPath];
			if ([self.delegate respondsToSelector:@selector(cycleScrollView:andCell:andCurrentpPageIndex:)]) {
				//实现这个代理方法进行赋值就可以了
				[self.delegate cycleScrollView:self andCell:aCell andCurrentpPageIndex:itemIndex];
			}
			return aCell;
		}
	}

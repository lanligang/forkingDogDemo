...
static const char LgCollectionHeaderKey = '\0';
...
#pragma mark 后添加的 增加一个 collctionHeader的方法
-(void)setLg_header:(CollectionViewHeader *)lg_header
{
	if (lg_header !=self.lg_header) {
		[self.lg_header removeFromSuperview];
		[self insertSubview:lg_header atIndex:0];
		// 存储新的
		[self willChangeValueForKey:@"lg_header"]; // KVO
		objc_setAssociatedObject(self, &LgCollectionHeaderKey,
								 lg_header, OBJC_ASSOCIATION_ASSIGN);
		[self didChangeValueForKey:@"lg_header"]; // KVO
	}
}
#pragma mark Getter
- (CollectionViewHeader *)lg_header
{
	return objc_getAssociatedObject(self, &LgCollectionHeaderKey);
}
